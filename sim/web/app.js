...
class Half {
  ...
  loadScene(scene) {
    ...
    for (const [i, l] of (scene.layers || []).entries()) {
      ...
      /* Set by the channel merge below, never by a scene written by hand:
       * it is how one channel's layers get their own brightness, speed and
       * hue without a render pass of their own. See mergedScene().
       */
      if (l._tune) {
        this.e.vfx_sim_set_layer_tune(rc, l._tune);
      }
    }
  }
...
/* Pushes one scene's zones and its own node -- everything below the engine
 * that a single channel (or, on a one-channel board, the whole export)
 * needs. `prefix` namespaces the zone and scene node names, which matters
 * once more than one channel's zones and scenes share a file: two channels
 * both naming a zone "all" would otherwise collide.
 */
function emitScene(out, prefix, scene) {
  const hsb = c => `VFX_HSB(${c[0]}, ${c[1]}, ${c[2]})`;
  const ind = '            ';
  const sceneId = `${prefix}${ident(scene.name)}`;

  out.push('        zones {');
  for (const [name, z] of Object.entries(scene.zones || {})) {
    const zoneId = `${prefix}${name}`;
    const body = Array.isArray(z.pixels)
      ? `pixels = <${z.pixels.join(' ')}>;`
      : Array.isArray(z.keys)
        ? `keys = <${z.keys.join(' ')}>;`
        : `range = <${z.range[0]} ${z.range[1]}>;`;
    out.push(`            ${zoneId}: ${zoneId} { compatible = "zmk,vfx-zone"; ${body} };`);
  }
  out.push('        };');
  out.push('');
  out.push(`        ${sceneId}: ${sceneId} {`);
  ...
  for (const [i, l] of (scene.layers || []).entries()) {
    ...
    out.push(`${ind}    zone = <&${prefix}${l.zone}>;`);
...
/* One channel's engine sub-node. On a one-channel board this only ever runs
 * once, over the channel that covers the whole strip, and its own comment
 * below explains why that case is still worth writing this way rather than
 * as the plain default-scene the engine also accepts.
 */
function emitChannel(out, ch, sceneId) {
  out.push(`        channel_${ident(ch.name)}: channel-${ch.name} {`);
  out.push('            compatible = "zmk,vfx-channel";');
  out.push(`            range = <${ch.range[0]} ${ch.range[1]}>;`);
  /* The composer only ever builds one scene per channel, so this list is
   * always one long; on hardware NEXT/PREVIOUS steps through as many as the
   * channel declares, which is not something the simulator's tabs offer.
   */
  out.push(`            scenes = <&${sceneId}>;`);
  out.push(`            default-scene = <&${sceneId}>;`);
  out.push('        };');
}

function toDevicetree(channels, ledsPerHalf) {
  const out = [];
  ...
  /* One channel covering the whole strip is written as a plain
   * default-scene, same as every export before channels existed, rather
   * than as a channel node that happens to own everything: the engine
   * treats a board with no channels this way already, so this is the
   * shorter devicetree for the common case, not a different feature.
   */
  if (channels.length === 1) {
    const [ch] = channels;

    out.push(`        default-scene = <&${ident(ch.scene.name)}>;`);
    out.push('    };');
    out.push('');
    out.push('    vfx_scenes {');
    emitScene(out, '', ch.scene);
    out.push('    };');
  } else {
    /* Each channel's prefix namespaces both its zones and its scene, so two
     * channels naming a zone or a preset the same thing cannot collide.
     */
    const prefixes = channels.map(ch => `${ident(ch.name)}_`);

    /* Declaration order is the channel id the keymap addresses, same as in
     * a real overlay -- see zmk-config's channel-glow/channel-keys.
     */
    channels.forEach((ch, i) => {
      out.push('');
      emitChannel(out, ch, `${prefixes[i]}${ident(ch.scene.name)}`);
    });
    out.push('    };');
    out.push('');
    out.push('    vfx_scenes {');
    channels.forEach((ch, i) => {
      emitScene(out, prefixes[i], ch.scene);
      out.push('');
    });
    out.pop();
    out.push('    };');
...
/* The scene a channel starts with, keyed by channel name. Not part of the
 * preset picker: these are two halves of what "Underglow and keys" used to
 * fake with zones in one scene, now genuinely separate channels each with
 * their own brightness, speed and hue as well as their own scene.
 */
const CHANNEL_DEFAULTS = {
  glow: structuredClone(PRESETS['Glow Pulse']),
  keys: {
    name: 'Keys reactive',
    zones: { all: { range: [0, 255] } },
    layers: [
      { type: 'ripple', zone: 'all', color: [190, 40, 100],
        decay_ms: 700, speed: 60, width: 15 },
    ],
  },
  all: structuredClone(PRESETS.Aurora),
};
...
async function main() {
  ...
  /* One slice of the strip carrying its own scene, brightness, speed, hue
   * and on/off, exactly as `struct vfx_channel` and `vfx_chan_state` do at
   * runtime. A board that declares none is one channel covering everything,
   * same as there.
   *
   * There is still only one wasm instance per physical half (`halves`,
   * below) rather than one per channel: every channel's layers are merged
   * into a single scene before it is loaded, each tagged with a tuning slot
   * of its own (see mergedScene()), which is how they end up with
   * independent brightness, speed and hue out of one render pass. That is a
   * simulator trick, not what ships — real firmware gives each channel its
   * own render call and copies out only the pixels it owns. It was chosen
   * because it reuses machinery already built, tested and linked in
   * (vfx_sim_tune / vfx_sim_set_layer_tune) rather than adding a second
   * wasm instance per channel with its own masking and power-gate logic.
   */
  let channels = [];
  let activeChannel = 0;

  const activeScene = () => channels[activeChannel].scene;

  function channelDefs() {
    if (stripPath === 'mixed') {
      return [
        { name: 'glow', range: [0, MIXED_GLOW] },
        { name: 'keys', range: [MIXED_GLOW, ledsPerHalf - MIXED_GLOW] },
      ];
    }

    return [{ name: 'all', range: [0, ledsPerHalf] }];
  }

  /* A layer written for the whole board (`range: [0, 255]`, the convention
   * every preset uses) is clipped to the channel's own window here, which is
   * what makes any preset droppable onto any channel and have it show up
   * only on that channel's LEDs. `pixels` zones are filtered the same way.
   * `keys` zones are not: they resolve through the engine's own key map,
   * which this merge cannot see, so a channel scene using one is trusted to
   * name keys that actually land inside its own window.
   */
  function clipZoneToChannel(zone, [start, len]) {
    const end = start + len;

    if (Array.isArray(zone.range)) {
      const a = Math.max(zone.range[0], start);
      const b = Math.min(zone.range[0] + zone.range[1], end);

      return { range: [a, Math.max(0, b - a)] };
    }

    if (Array.isArray(zone.pixels)) {
      return { pixels: zone.pixels.filter(p => p >= start && p < end) };
    }

    return zone;
  }

  /* One combined scene out of every channel that is on, each channel's zones
   * and layers namespaced so two channels naming a zone "all" cannot collide,
   * and each layer tagged with a tuning slot (1-based; slot 0 means untuned)
   * so the per-channel brightness/speed/hue applied in drawFrame() reaches
   * only that channel's own layers.
   */
  function mergedScene() {
    ...
    channels.forEach((ch, ci) => {
      if (!ch.state.on) return;

      for (const [name, z] of Object.entries(ch.scene.zones ?? {})) {
        zones[`ch${ci}_${name}`] = clipZoneToChannel(z, ch.range);
      }

      for (const l of ch.scene.layers ?? []) {
        layers.push({ ...l, zone: `ch${ci}_${l.zone}`, _tune: ci + 1 });
      }
    });

    return { name: 'combined', zones, layers };
  }
  ...
  function reinit() {
    ...
    /* Rebuilt from the defs every time rather than diffed, but a channel
     * that survives under the same name (switching LEDs-per-half without
     * changing strip path, say) keeps its scene and its sliders rather than
     * losing them to the default.
     */
    channels = channelDefs().map(d => {
      const prev = channels.find(c => c.name === d.name);
      ...
      return prev
        ...
        : {
            name: d.name,
            range: d.range,
            scene: structuredClone(CHANNEL_DEFAULTS[d.name] ?? CHANNEL_DEFAULTS.all),
            state: { brightness: 255, speed: 3, hue: 0, on: true },
          };
    });
    if (activeChannel >= channels.length) activeChannel = 0;
  ...
  function applyScene() {
    try {
      ...
      for (const h of halves) h.loadScene(merged);
  ...
  function layerCard(layer, i) {
    ...
    button('✕', 'Remove', () => { activeScene().layers.splice(i, 1); composed(); });
    ...
    field('zone', select(layer.zone, Object.keys(activeScene().zones ?? {}),
                         v => { layer.zone = v; }));
  ...
  function renderComposer() {
    ...
    list.innerHTML = '';
    (layers ?? []).forEach((l, i) => list.appendChild(layerCard(l, i)));
  ...
  /* Buttons for switching which channel the composer, the presets and the
   * scene JSON below are all editing. A board with one channel gets no row
   * at all: the common case looks exactly like it did before channels.
   */
  function renderChannelTabs() {
    const row = $('channel-tabs');

    row.innerHTML = '';
    /* Inline style rather than the hidden attribute: .row sets display:flex,
     * which in the cascade outranks the UA stylesheet's [hidden] rule.
     */
    row.style.display = channels.length < 2 ? 'none' : '';
    $('channel-note').style.display = channels.length < 2 ? 'none' : '';

    channels.forEach((ch, ci) => {
      const b = document.createElement('button');

      b.textContent = ch.name + (ch.state.on ? '' : ' (off)');
      b.className = 'preset' + (ci === activeChannel ? ' active' : '');
      b.title = `pixels ${ch.range[0]}-${ch.range[0] + ch.range[1] - 1}`;
  ...
  /* Points the Engine panel's brightness/speed/hue sliders and the on/off
   * checkbox at whichever channel is active, so they read and write that
   * channel's own state rather than a board-wide one.
   */
  function syncChannelControls() {
    const st = channels[activeChannel].state;

    $('brightness').value = st.brightness;
    $('out-brightness').textContent = `${Math.round(st.brightness / 255 * 100)}%`;
    $('speed').value = st.speed;
    $('out-speed').textContent = st.speed;
    $('hue').value = st.hue;
    $('out-hue').textContent = `${st.hue}°`;
    $('chan-on').checked = st.on;
  }
  ...
  function drawFrame(t) {
    ...
    for (const h of halves) {
      /* Every layer in the merged scene carries a tuning slot (see
       * mergedScene()), so board-wide brightness/speed/hue is left neutral
       * here and each channel's own slot below is what actually reaches it.
       */
      h.setState(255, 3, 0);

      channels.forEach((ch, ci) => {
        h.e.vfx_sim_tune(ci + 1, ch.state.hue, ch.state.brightness, ch.state.speed);
      });
  ...
  /* Brightness, speed and hue belong to the active channel rather than to
   * the board, so these write into channels[activeChannel].state instead of
   * bindRange's shared `state` object; drawFrame() reads it every frame.
   */
  const bindChannelRange = (id, key, fmt = v => v) => {
    const el = $(id), out = $('out-' + id);
    el.addEventListener('input', () => {
      channels[activeChannel].state[key] = Number(el.value);
      out.textContent = fmt(Number(el.value));
    });
  };

  bindChannelRange('brightness', 'brightness', v => `${Math.round(v / 255 * 100)}%`);
  bindChannelRange('speed', 'speed');
  bindChannelRange('hue', 'hue', v => `${v}°`);

  $('chan-on').addEventListener('change', () => {
    channels[activeChannel].state.on = $('chan-on').checked;
    renderChannelTabs();
    applyScene();
  });
  ...
  $('fade-black').addEventListener('click', () => {
    /* Ramps the active channel's brightness down rather than snapping, so
     * the gate is seen to wait out its delay instead of firing the instant
     * the value hits zero. On a board with more than one channel this only
     * gates the rail once every channel has been faded, same as on hardware.
     */
    const el = $('brightness');
    const st = channels[activeChannel].state;
    const from = st.brightness;
    const t0 = performance.now();
    const step = now => {
      const k = Math.min(1, (now - t0) / 900);
      st.brightness = Math.round(from * (1 - k));
      el.value = st.brightness;
      $('out-brightness').textContent = `${Math.round(st.brightness / 255 * 100)}%`;
  ...
  $('path').addEventListener('change', e => {
    ...
    /* Also rebuilds the channel list: leaving mixed drops back to one
     * channel covering everything, and reinit() is what notices that.
     */
    reinit();
  });
  ...
  $('apply').addEventListener('click', () => {
    try {
      channels[activeChannel].scene = JSON.parse($('scene').value);
    } catch (err) {
  ...
  $('export').addEventListener('click', async () => {
    const dt = toDevicetree(channels, ledsPerHalf);
    try {
  ...
  for (const name of Object.keys(PRESETS)) {
    ...
    b.className = 'preset';
    b.addEventListener('click', () => {
      channels[activeChannel].scene = structuredClone(PRESETS[name]);
      $('scene').value = JSON.stringify(activeScene(), null, 2);
      renderComposer();
      applyScene();
  ...
  /* Builds `channels` from scratch and refreshes the composer and scene JSON
   * to match, which is also why it has to run before anything else here
   * reads activeScene().
   */
  reinit();
  ...
  window.vfxDebug = {
    ...
    /* Channel state and the tab switch, for tests that need to look at or
     * drive one channel independently of whichever is active in the UI.
     */
    channels: () => channels,
    selectChannel,
  };
